...
function setCurrentDate() {
    const now = new Date();
    const formattedDate = now.toUTCString().slice(5, 25); // Format as "Mon, DD Mon YYYY HH:MM:SS"
...
function dateToHoursSinceEpoch(dateString) {
    const date = new Date(Date.parse(dateString + " UTC")); // Parse in UTC
    ...
    const millisecondsSinceEpoch = date.getTime() - Date.UTC(1, 0, 1); // UTC Jan 1, year 1
