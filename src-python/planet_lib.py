...
def print_planet_time(date, day_len=0):
    """
    ...
    if day_len < 100:
        ...
        print(f"Year {date[0]}, day {date[1]}, " + "{:04}:{:02}:{:02}".format(date[2], date[3], floor(date[4])))
    else:
        print(f"Year {date[0]}, day {date[1]}, " + "{:03}:{:02}:{:02}".format(date[2], date[3], floor(date[4])))

spectime = 17739736.883
date = hours_to_earth(spectime)
print_earth_time(date)
print(hours_to_mars(spectime))
print_planet_time(hours_to_mars(spectime), 11)